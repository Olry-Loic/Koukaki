<?php
/**
 * Fleurs d\'oranger & Chats errants functions and definitions
 *
...
function foce_setup() {
	/*
		...
		*/
	load_theme_textdomain( 'foce', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'foce' ),
		)
	);
 ...
 */
function foce_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'foce_content_width', 640 );
}
add_action( 'after_setup_theme', 'foce_content_width', 0 );
 ...
 */
function foce_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'foce' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'foce' ),
...
add_action( 'widgets_init', 'foce_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function foce_scripts() {
	wp_enqueue_style( 'foce-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'foce-style', 'rtl', 'replace' );

	wp_enqueue_script( 'foce-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'foce_scripts' );
